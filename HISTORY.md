...
## Session — 2026-05-17 (Claude Code Memory Flywheel V1)

**What changed**:
- Implemented the full design in `docs/superpowers/specs/2026-05-17-claude-code-memory-flywheel-design.md`: two-layer markdown memory (`context/STATE.md` + `context/DECISIONS.md` project layer, `~/.claude/projects/.../memory/` personal layer), SessionEnd auto-capture hook with Haiku summarisation, SessionStart opportunistic catch-up, tier-0/1/2/3 retrieval contract codified in `CLAUDE.md`.
- New API endpoint `GET /v1/memory/{id}/expand` returning full content + parent `RawMemory.raw_text` + 3 nearest same-source neighbors; new `expand_memory()` service in `src/api/services/memory_service.py` with cross-DB-safe neighbor query (two windowed lookups + Python sort by absolute time delta, avoiding Postgres-only `EXTRACT(EPOCH FROM ...)`). New `memory_expand` MCP tool with markdown-formatted output (`src/mcp_server.py`).
- SessionEnd hook (`scripts/claude-code/session-end-ingest.sh` bash wrapper + `session_end_ingest.py` worker) with backend pluggability: `OB_SESSION_END_BACKEND=cli` (default — `claude --print`, subscription) / `api` (REST + `ANTHROPIC_API_KEY`, pay-per-token) / `auto`. Reads SessionEnd JSON payload from stdin, parses JSONL transcript (last 150k chars), idempotently creates `project_labels`, appends to `context/sessions/{date}.md`, POSTs to `/v1/memory` with `source="claude-code-session"` + `metadata.project`.
- SessionStart hook (`scripts/claude-code/session-start-distill.sh`) — self-scopes by `cwd` match against script's repo, fires `run-distill.sh` in background only on `source=startup`, throttled to once per `OB_SESSION_START_MIN_HOURS` (default 6h), `flock`-protected for parallel CC windows.
- Local-WSL cron infrastructure: `scripts/memory/run-distill.sh`, `run-curate.sh`, `state.py` inspector, `install-anacron.sh` (writes `/etc/cron.d/ob-memory-flywheel` + anacron entries); `make memory-state/distill/curate/logs/tail/clean-logs/install-cron/uninstall-cron`.
- Sentinel-anchored catch-up: `context/.last-distill` tracks last successful distill date; daily job processes every session file with date > sentinel oldest→newest. Made `cron/jobs/daily-memory-distill.md` explicit about this.
- Added `claude-code-session` to `AUTO_CAPTURE_SOURCES` in `src/pipeline/constants.py` so the SessionEnd-ingested rows get importance-capped and skip Task creation.
- 17 new tests (11 in `tests/test_memory_expand.py` for service + HTTP route + neighbor ordering + 404/422; 6 in `tests/test_mcp_server.py` for `_do_memory_expand`).

**Files touched**: `src/api/services/memory_service.py`, `src/api/routes/memory.py`, `src/mcp_server.py`, `src/pipeline/constants.py`, `tests/test_memory_expand.py` (new), `tests/test_mcp_server.py`, `scripts/claude-code/session-end-ingest.sh` (new), `scripts/claude-code/session_end_ingest.py` (new), `scripts/claude-code/session-start-distill.sh` (new), `scripts/memory/run-distill.sh` (new), `scripts/memory/run-curate.sh` (new), `scripts/memory/state.py` (new), `scripts/memory/install-anacron.sh` (new), `context/STATE.md` (new), `context/DECISIONS.md` (new), `cron/jobs/daily-memory-distill.md` (new), `cron/jobs/weekly-memory-curator.md` (new), `docs/setup-new-machine.md` (new), `CLAUDE.md`, `Makefile`, `.gitignore`, `PROGRESS.md`

**Decisions made**: Local-only cron over `/schedule` remote (remote runtime can't see `~/.claude/.../memory/` or `context/sessions/`). Backend pluggability on SessionEnd hook (cli vs api) so user can switch between subscription and pay-per-token without code change. SessionStart hook self-scoped via cwd-vs-script-repo match — safe to globally install. Sentinel-file catch-up over "process exactly today" — makes the system tolerant to laptop-off-at-23:00. Neighbor query uses Python sort over two windowed lookups to stay cross-DB; `_as_utc_timestamp()` coerces naive SQLite vs tz-aware Postgres datetimes uniformly. `expand_memory` returns 404 (not 410) for superseded — caller should not chase outdated entries.

**Gotchas found**: SQLite stores datetimes naive while Postgres tz-aware — subtracting them in tests raises `TypeError`; fix is the `_as_utc_timestamp()` helper. `/schedule` slash command runs on Anthropic-hosted runtime, not the laptop, so it can't reach local filesystem — the `cron/jobs/*.md` YAML files describe behaviour but the real scheduler is local cron + anacron. WSL2 doesn't start `cron` daemon by default — needs `/etc/wsl.conf` `[boot] command = "service cron start"`. SessionStart hook's `source` field distinguishes `startup`/`resume`/`clear`/`compact` — only `startup` should trigger fresh work. The current personal memory layer is severely over caps (some files >16× cap) — first `make memory-curate` will have substantial consolidation to do.

**Test count**: 859 backend (844 → 859, +15 net from +17 new memory tests; existing pass count unchanged) | ~303 Vitest (unchanged)
