...
- **Deploy** all pending changes (migrations 0016–0021 + Learning V2 + Commitments tab + plan import sets + Discord removal + Learning cron cross-day dedup + topic context on TodoResponse + **memory_expand endpoint + `claude-code-session` source in `AUTO_CAPTURE_SOURCES`** + **exercise library + per-day schedule + import wizard + plan CRUD** — `src/api/routes/memory.py`, `src/api/services/memory_service.py`, `src/mcp_server.py`, `src/pipeline/constants.py`, `src/core/models.py`, `src/api/services/commitment_import_service.py`, `src/api/services/schedule_service.py`, `src/api/routes/commitments.py`, `src/api/routes/exercises.py`, `alembic/versions/0020_exercise_library.py`, `alembic/versions/0021_commitment_entry_exercises.py`) — `git pull` on GCP VM then `docker compose --profile migrate run --rm migrate` + restart services; remove `discord-bot` container if running (`docker rm -f openbrain-discord`)
...
- **Spec A** (inline exercise log form + day-swap from commitment detail page) — see `docs/superpowers/specs/active/2026-05-18-commitment-spec-a-log-form-day-swap.md`
