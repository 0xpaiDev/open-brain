...
- **JSX text — use literal `&`, not `&amp;`** — `&amp;` in JSX text content is a double-escape; browsers render it as the four-character string `&amp;`, not `&`. Write a literal `&` between words in JSX (e.g. `Trace & cost visibility`). The linter or code generation may insert `&amp;`; fix it manually if so.
- **React hooks return new object refs every render** — `const traces = useTraces(filters)` gives a new object reference each render. Depending on `traces` or `kpis` in `useCallback`/`useEffect` deps resets the effect on every render (e.g. clears `setInterval` before it fires). Destructure stable function refs: `const { refresh: refreshTraces, ...rest } = useTraces(filters)` and list `refreshTraces` in deps, not `traces` (`web/app/logs/page.tsx`).
