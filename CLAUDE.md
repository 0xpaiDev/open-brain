...
- **Learning cron has TWO orthogonal dedup checks**: `src/jobs/learning_daily.py::run_learning_selection` runs (a) same-day gate via `_count_todays_learning_todos()` — "did cron already fill today's slot?" — and (b) cross-day item filter via `_open_learning_item_ids()` — every `learning_item` with at least one OPEN todo (any date) is excluded from selection so the same item never gets a duplicate todo while the previous one is still in flight. `done`/`cancelled` todos do NOT block re-pick. Both `POST /v1/learning/refresh` and the cron share this logic. Fallback when LLM fails is DETERMINISTIC (oldest pending items sorted by `created_at + id`), never random.
- **TodoItem.learning_item uses lazy='raise'**: `src/core/models.py` declares the FK relationship with `lazy="raise"` so any async lazy-load attempt becomes a loud sync error instead of a silent `MissingGreenlet`. Every read site that calls `_todo_to_response()` (`src/api/routes/todos.py`) must eager-load via `_with_learning_topic()` selectinload chain (`TodoItem.learning_item → LearningItem.section → LearningSection.topic`). After `update_todo()` the relationship is expired (refresh wipes it) — PATCH and bulk-defer must re-fetch with `_with_learning_topic()` before serializing.
