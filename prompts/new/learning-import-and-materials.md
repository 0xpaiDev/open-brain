...
5. If not `dry_run`:
   - Get current max `position` from `learning_topics`.
   - For each `ImportTopic` in `to_create`, create `LearningTopic` with `position = max + idx`, then add sections (`position=idx`), then items (`position=idx`), then `LearningMaterial` if present.
...
- `GET /v1/learning/topics/{topic_id}/material` — returns `MaterialOut` or `null` (200 with body `null` if topic exists but has no material; 404 if topic missing).
...
10. `test_import_rate_limit_5_per_minute` — fire 6 requests, the 6th returns 429 (mirrors existing rate-limit tests in `tests/test_rate_limits.py` if present, else inline pattern).
...
Refactor: extract `SectionBlock` and `ItemRow` from [web/app/learning/page.tsx](web/app/learning/page.tsx) into `web/app/learning/_components/section-block.tsx` and `web/app/learning/_components/item-row.tsx`. Both pages (tree + topic detail) reuse them. Pure code move, no behavior change.
